...
elif CFG.PROMPT_TYPE == "vinallama":
    QA_TEMPLATE = _vinallama_format.format(system=QA.system, user=QA.user)
    HYDE_TEMPLATE = _vinallama_format.format(system=Hyde.system, user=Hyde.user)
    MULTI_QUERIES_TEMPLATE = _vinallama_format.format(
        system=MultipleQueries.system, user=MultipleQueries.user
    )
